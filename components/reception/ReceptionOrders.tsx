...
import { Coffee, Utensils, CheckCircle, Clock, RefreshCcw } from 'lucide-react';
import { mockReceptionOrdersData } from '@/lib/mocks/receptionOrdersMock';

export default function ReceptionOrders() {
  const [data, setData] = useState<any>(mockReceptionOrdersData.data);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (json.success && json.data.pendingFoodOrders?.length > 0) {
        ...
        setData(mockReceptionOrdersData.data);
      }
    } catch (err) {
      console.error(err);
      setData(mockReceptionOrdersData.data);
  ...
  const orders = data?.pendingFoodOrders || [];
          ...
          orders.map((order: any, idx: number) => (
            ...
            >
              ...
              <div className="relative flex justify-between items-center pt-4 border-t border-slate-700">
                ...
                <button className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-1.5 rounded-lg transition-all duration-300 text-sm font-semibold" style={{ boxShadow: '0 2px 8px rgba(5, 150, 105, 0.2)' }}>
